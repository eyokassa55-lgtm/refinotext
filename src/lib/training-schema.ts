 ...
 * memory (TF-IDF vectors + inverted index). Neon stores user requests in
 * `humanizations`, not the training pairs — no extra billing/auth tables.
 *
 * Column mapping:
...
export const TRAINING_EXAMPLE_COLUMNS = {
  ...
  human_text: "text not null — stored rewrite, returned unchanged on a database hit",
} as const;
